...
public class BaseTest extends TestCase {
	...
	public void saveAddQuestion(WebDriver driver, String type, JavascriptExecutor jse) throws InterruptedException {
		...
		Thread.sleep(1000);
		jse.executeScript("$(\".searchT\").find(\"select option[value='" + type
				+ "']\").prop(\"selected\", true).trigger(\"change\")"); // 選擇題型
		switchAndScroll(driver, jse, 500);
		Thread.sleep(1000);
		jse.executeScript(
				"$(\".searchT\").find(\"select option[value=aut_003]\").prop(\"selected\", true).trigger(\"change\")"); // 授權模式
		jse.executeScript("$(\"#areaTypeUL > li:nth-child(3)\").click()"); // 公開狀態
		jse.executeScript("$(\"#diffcultUL > li:nth-child(2)\").click()"); // 難易度
		Thread.sleep(1000);
		jse.executeScript("$(\"#cke_questionContent iframe\").contents().find(\"html body\").html(\"QuestionTest Add_"
				+ type + "\")"); // 題目
		if (type.equals(Constant.CHOICE_QUESTION) || type.equals(Constant.SELECTION_QUESTION)
				|| type.equals(Constant.MATCH_QUESTION)) {
		...
		} else if (type.equals(Constant.SHORT_ANS_QUESTION) || type.equals(Constant.BLANK_FILL_QUESTION)
				|| type.equals(Constant.ESSAY_QUESTION) || type.equals(Constant.MATH_QUESTION)) {
			jse.executeScript(
					"$(\"#cke_answer_Content iframe\").contents().find(\"html body\").html(\"Answer_" + type + "\")");
		}
	...
	public void addOptionByType(WebDriver driver, String type, JavascriptExecutor jse) throws InterruptedException {
		String searchElement = (type.equals(Constant.MATCH_QUESTION) ? "#matchTable > tbody > tr > td > .ckeditor"
				: "#question_option > .ckeditor");
		...
		for (int i = 0; i < size; i++) {
			...
			jse.executeScript("$(\"#cke_" + queContent + "_" + i
					+ " iframe\").contents().find(\"html body\").html(\"Answer_" + i + "\")");
			Thread.sleep(1000);
			if (type.equals(Constant.MATCH_QUESTION)) {
				jse.executeScript("$(\"#cke_m_ansContent_" + i
						+ " iframe\").contents().find(\"html body\").html(\"Match_Answer_" + i + "\")");
			}
		}
	...
	public void editQuestion(WebDriver driver, String type, JavascriptExecutor jse) throws InterruptedException {
		...
		Thread.sleep(1000);
		jse.executeScript(
				"$(\".searchT\").find(\"select option[value=aut_001]\").prop(\"selected\", true).trigger(\"change\")"); // 授權模式
		...
		Thread.sleep(1000);
		jse.executeScript("$(\"#cke_questionContent iframe\").contents().find(\"html body\").html(\"QuestionTest Edit_"
				+ type + "\")"); // fill content
		Thread.sleep(1000);
		jse.executeScript("goSave()");
	}
	...
	public void copyQuestion(WebDriver driver, String type, JavascriptExecutor jse) throws InterruptedException {
		...
		Thread.sleep(1000);
		jse.executeScript(
				"$(\".searchT\").find(\"select option[value=aut_002]\").prop(\"selected\", true).trigger(\"change\")"); // 授權模式
		jse.executeScript("$(\"#areaTypeUL > li:nth-child(2)\").click()"); // 公開狀態
		jse.executeScript("$(\"#diffcultUL > li:nth-child(2)\").click()"); // 難易度
		jse.executeScript("$(\"#cke_questionContent iframe\").contents().find(\"html body\").empty()"); // reset content
		Thread.sleep(1000);
		jse.executeScript("$(\"#cke_questionContent iframe\").contents().find(\"html body\").html(\"QuestionTest Copy_"
				+ type + "\")"); // fill content
		Thread.sleep(1000);
		jse.executeScript("goSave()");
	}
